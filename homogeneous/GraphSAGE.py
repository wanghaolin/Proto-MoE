...
class TrueDeepSAGEEncoder(nn.Module):
...
class StandardSAGEExpert(nn.Module):
    def __init__(self, input_dim, output_dim=2, hidden_dim=64, num_layers=2, dropout=0.3, expert_name="SAGEExpert"):
        super().__init__()
        self.expert_type = expert_name
        self.expert_confidence = 1.0
        self.dropout = dropout
...
# [Main Model]
# ==========================================
class OptimizedSAGEProtoMoE(nn.Module):
    def __init__(self, input_dim, out_channels=128, num_prototypes=10, num_experts=None, top_k=None,
        ...
        self.encoder = TrueDeepSAGEEncoder(
            input_dim=input_dim, hidden_channels=SAGE_HIDDEN, out_channels=embedding_dim,
        ...
        self.experts = nn.ModuleList([
            ...
            StandardSAGEExpert(input_dim, expert_output_dim, hidden_dim=64, num_layers=2, expert_name="SAGE_Shallow"),
            StandardSAGEExpert(input_dim, expert_output_dim, hidden_dim=64, num_layers=3, expert_name="SAGE_Deep"),
    ...
    def forward(self, data, edge_dropout=0.0):
        ...
        for exp in self.experts:
            ...
            # Graph Experts (SAGE)
            elif isinstance(exp, StandardSAGEExpert):
...
def evaluate_sage_proto_moe(model, data):
    model.eval()
    with torch.no_grad():
        out = model(data)
...
def enhanced_three_stage_training(model, train_data, epochs_per_stage=500, lr=0.001):
    ...
    for epoch in range(1, epochs_per_stage + 1):
        ...
        if epoch % 10 == 0:
            val_metrics = evaluate_sage_proto_moe(model, train_data)
    ...
    return evaluate_sage_proto_moe(model, train_data)


def train_sage_proto_moe_cv(train_data, save_dir='model', n_folds=5, epochs_per_stage=500, dynamic_k=None):
...
def find_optimal_prototypes_elbow(data, input_dim, device, output_dir, k_range=(2, 21)):
    temp_encoder = TrueDeepSAGEEncoder(
...
def main_sage_proto_moe():
    CONFIG = {
        'train_graph_path': 'train_graph-homo.pt',
        'valid_graph_path': 'valid_graph-homo.pt',
        'output_dir': 'F:\\SAGEProtoMoE',
        'model_save_dir': 'F:\\SAGEProtoMoE\\models',
        'prototype_export_dir': 'F:\\SAGEProtoMoE\\prototype_samples',
        'num_prototypes': 10,
    ...
    try:
        ...
        cv_res = train_sage_proto_moe_cv(
        ...
        best_fold_model = OptimizedSAGEProtoMoE(
        ...
        print("\n=== Final Full Training ===")
        final_model = OptimizedSAGEProtoMoE(
            input_dim=input_dim, out_channels=128,
            num_prototypes=CONFIG['num_prototypes'], device=device
        ).to(device)
        ...
        save_name = 'SAGE_predictions.npz'
