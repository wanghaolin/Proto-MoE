...
class NativeTabNet(nn.Module):
    ...
    def forward(self, x):
        ...
        for step in range(self.n_steps):
            ...
            M_loss += torch.mean(torch.sum(-M * torch.log(M + 1e-8), dim=1))
...
# 1. Feature Tokenizer (for TabTransformer)
...
# 2. Full-Design TabTransformer Expert
...
class DeepTablesExpert(nn.Module):
    def __init__(self, input_dim, output_dim=2, hidden_dims=[256, 128, 64], cross_layers=3, dropout=0.3):
        super().__init__()
        self.expert_type = "DeepTables"
        self.expert_confidence = 1.0
        self.input_dim = input_dim
        self.wide = nn.Linear(input_dim, hidden_dims[-1])
        self.cross_layers = nn.ModuleList([nn.Linear(input_dim, input_dim) for _ in range(cross_layers)])
        deep_layers = []
        prev_dim = input_dim
        for hidden_dim in hidden_dims:
            deep_layers.extend([nn.Linear(prev_dim, hidden_dim), nn.LayerNorm(hidden_dim), nn.ReLU(), nn.Dropout(dropout)])
            prev_dim = hidden_dim
        self.deep = nn.Sequential(*deep_layers)
        fusion_dim = hidden_dims[-1] + input_dim + hidden_dims[-1]
        self.fusion = nn.Sequential(nn.Linear(fusion_dim, hidden_dims[-1]), nn.ReLU(), nn.Dropout(dropout),
    ...
    def forward(self, x):
        ...
        concat = torch.cat([wide_out, cross_out, deep_out], dim=1)
...
class FTTransformerExpert(nn.Module):
    def __init__(self, num_features, output_dim=2, dim=64, depth=3, heads=8, dropout=0.3):
        ...
        self.cls_token = nn.Parameter(torch.randn(1, 1, dim))
        self.transformer_layers = nn.ModuleList([FTTransformerBlock(dim, heads, dropout=dropout) for _ in range(depth)])
        self.head = nn.Sequential(nn.LayerNorm(dim), nn.Linear(dim, dim // 2), nn.GELU(), nn.Dropout(dropout),
...
class TrueDeepGINEncoder(nn.Module):
...
class StandardGINExpert(nn.Module):
    def __init__(self, input_dim, output_dim=2, hidden_dim=64, num_layers=2, dropout=0.3, expert_name="GINExpert"):
        super().__init__()
        self.expert_type = expert_name
        self.expert_confidence = 1.0
        self.dropout = dropout
...
class StabilizedPrototypeGuidedRouter(nn.Module):
    def __init__(self, input_dim, num_experts, num_prototypes, top_k=3, hidden_dim=64, dropout=0.3):
        super().__init__()
        self.num_experts = num_experts;
        self.top_k = top_k
        self.prototype_to_expert_map = nn.Parameter(torch.randn(num_prototypes, num_experts) * 0.5)
        self.router_net = nn.Sequential(
            nn.Linear(input_dim + num_prototypes, hidden_dim), nn.ReLU(), nn.LayerNorm(hidden_dim), nn.Dropout(dropout),
    ...
    def forward(self, embeddings, proto_sims):
        ...
        if self.training:
            ...
            updated_ema = self.usage_decay * self.expert_usage_ema + (1 - self.usage_decay) * probs.mean(0).detach()
...
class FullWeightRouter(nn.Module):
    def __init__(self, input_dim, num_experts, num_prototypes, hidden_dim=64, dropout=0.3):
        ...
        self.router_net = nn.Sequential(nn.Linear(input_dim + num_prototypes, hidden_dim), nn.ReLU(),
                                         nn.LayerNorm(hidden_dim), nn.Dropout(dropout), nn.Linear(hidden_dim, num_experts))
        self.register_buffer('expert_usage_ema', torch.ones(num_experts) / num_experts)
        self.usage_decay = 0.99
    ...
    def forward(self, embeddings, proto_sims):
        ...
        if self.training:
            updated_ema = self.usage_decay * self.expert_usage_ema + (1 - self.usage_decay) * expert_weights.mean(0).detach()
            self.expert_usage_ema.copy_(updated_ema)
...
class ConcatenationAggregator(nn.Module):
    def __init__(self, embedding_dim, expert_output_dim, top_k, hidden_dim=64, dropout=0.2):
        super().__init__()
        self.input_dim = embedding_dim + top_k * expert_output_dim
        self.mlp = nn.Sequential(
            nn.Linear(self.input_dim, hidden_dim), nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(hidden_dim, expert_output_dim)
        )

    def forward(self, z, expert_outs, expert_weights):
        weighted_outs = expert_outs * expert_weights.unsqueeze(-1)
        flattened_outs = weighted_outs.view(z.size(0), -1)
        agg_input = torch.cat([z, flattened_outs], dim=1)
        return self.mlp(agg_input)
...
class EnhancedMultiTaskLoss(nn.Module):
    def __init__(self, num_tasks, device='cuda', task_names=None):
        super().__init__()
        self.log_vars = nn.Parameter(torch.zeros(num_tasks, device=device))
        self.task_names = task_names

    def forward(self, losses, stage=3):
        total_loss = 0;
        log = {}
        for i, loss in enumerate(losses):
            ...
            log[f'{self.task_names[i]}_raw'] = loss.item() if isinstance(loss, torch.Tensor) else loss
...
class LoadBalancingLoss(nn.Module):
    ...
    def forward(self, expert_logits):
        probs = F.softmax(expert_logits / self.temperature, dim=1)
        usage = torch.mean(probs, dim=0)
        target = torch.ones(self.num_experts, device=expert_logits.device) / self.num_experts
        return F.kl_div(torch.log(usage + 1e-8), target, reduction='batchmean')
...
class OptimizedGINProtoMoE(nn.Module):
    def __init__(self, input_dim, out_channels=128, num_prototypes=10, num_experts=None, top_k=None,
        ...
        # 1. Main Encoder (TrueDeepGIN)
        self.encoder = TrueDeepGINEncoder(
            input_dim=input_dim, hidden_channels=GIN_HIDDEN, out_channels=embedding_dim,
        ...
        # 3. Experts Definition
        self.experts = nn.ModuleList([
            # General Experts (Operating on Embeddings)
            ...
            # Graph Experts (Operating on Graph Structure - Raw Features + Edge Index)
            StandardGINExpert(input_dim, expert_output_dim, hidden_dim=64, num_layers=2, expert_name="GIN_Shallow"),
            StandardGINExpert(input_dim, expert_output_dim, hidden_dim=64, num_layers=3, expert_name="GIN_Deep"),

            # Tabular Experts (Operating on Raw Features as Table)
            FullTabTransformerExpert(num_features=input_dim, output_dim=expert_output_dim),
            TabNetPackageExpert(input_dim=input_dim, output_dim=expert_output_dim)
        ])
    ...
    def forward(self, data, edge_dropout=0.0):
        ...
        for exp in self.experts:
            # Table Experts -> Raw Features
            if isinstance(exp, (FullTabTransformerExpert, TabNetPackageExpert)):
                all_outs.append(exp(raw_x))
            # Graph Experts -> Raw Features + Structure
            elif isinstance(exp, StandardGINExpert):
                all_outs.append(exp(raw_x, data.edge_index))
            # Embedding Experts -> Learned Embeddings
            else:
                all_outs.append(exp(embeddings))

        # 4. Full Weighted Aggregation
...
def evaluate_on_validation_set(model, valid_data, save_path=None):
    ...
    with torch.no_grad():
        try:
            ...
            # For independent validation, use all nodes
...
def train_gin_proto_moe_cv(train_data, save_dir='model', n_folds=5, epochs_per_stage=500, dynamic_k=None):
...
def find_optimal_prototypes_elbow(data, input_dim, device, output_dir, k_range=(2, 21)):
    ...
    temp_encoder = TrueDeepGINEncoder(
...
def main_gin_proto_moe():
    CONFIG = {
        'train_graph_path': 'train_graph-homo.pt',
        'valid_graph_path': 'valid_graph-homo.pt',
        'output_dir': 'GINProtoMoE',
        'model_save_dir': 'GINProtoMoE\\models',
        'prototype_export_dir': 'prototype_samples',
        'num_prototypes': 10,
    ...
    try:
        ...
        best_fold_model = OptimizedGINProtoMoE(
        ...
        print("\n=== Final Full Training ===")
        final_model = OptimizedGINProtoMoE(
            input_dim=input_dim, out_channels=128,
            num_prototypes=CONFIG['num_prototypes'], device=device
        ).to(device)

        train_data.train_mask = torch.ones(train_data.num_nodes, dtype=torch.bool, device=device)
        train_data.val_mask = train_data.train_mask
