...
# 1. Feature Tokenizer (for TabTransformer / FT-Transformer)
...
# GAT Encoder
# ==========================================
class TrueDeepGATEncoder(nn.Module):
    def __init__(self, input_dim, hidden_channels, out_channels, num_layers=3, dropout=0.3, heads=4):
        super().__init__()
        ...
        for _ in range(num_layers):
            # Using GATConv
            self.convs.append(GATConv(hidden_channels, self.head_dim, heads=heads, dropout=dropout, concat=True))
...
# GAT Expert
# ==========================================
class StandardGATExpert(nn.Module):
    def __init__(self, input_dim, output_dim=2, hidden_dim=64, num_layers=2, dropout=0.3, heads=4, expert_name="GATExpert"):
        super().__init__()
        self.expert_type = expert_name
        self.expert_confidence = 1.0
        self.dropout = dropout
        ...
        for _ in range(num_layers):
            self.convs.append(GATConv(hidden_dim, self.head_dim, heads=heads, dropout=dropout, concat=True))
...
class OptimizedGATProtoMoE(nn.Module):
    def __init__(self, input_dim, out_channels=128, num_prototypes=10, num_experts=None, top_k=None,
        ...
        GAT_HIDDEN = 128
        self.encoder = TrueDeepGATEncoder(
            input_dim=input_dim, hidden_channels=GAT_HIDDEN, out_channels=embedding_dim,
            num_layers=3, dropout=dropout, heads=heads
        )
        ...
        self.experts = nn.ModuleList([
            ...
            StandardGATExpert(input_dim, expert_output_dim, hidden_dim=64, num_layers=2, heads=heads, expert_name="GAT_Shallow"),
            StandardGATExpert(input_dim, expert_output_dim, hidden_dim=64, num_layers=3, heads=heads, expert_name="GAT_Deep"),
    ...
    def forward(self, data, edge_dropout=0.0):
        ...
        for exp in self.experts:
            ...
            # Graph Experts (GAT)
            elif isinstance(exp, StandardGATExpert):
...
# ==========================================
def evaluate_gat_proto_moe(model, data):
    model.eval()
    with torch.no_grad():
        out = model(data)
...
def enhanced_three_stage_training(model, train_data, epochs_per_stage=500, lr=0.001):
    ...
    for epoch in range(1, epochs_per_stage + 1):
        ...
        scheduler.step()
        if epoch % 10 == 0:
            val_metrics = evaluate_gat_proto_moe(model, train_data)
    ...
    return evaluate_gat_proto_moe(model, train_data)


def train_gat_proto_moe_cv(train_data, save_dir='model', n_folds=5, epochs_per_stage=500, dynamic_k=None):
...
def find_optimal_prototypes_elbow(data, input_dim, device, output_dir, k_range=(2, 21)):
    temp_encoder = TrueDeepGATEncoder(
...
def main_gat_proto_moe():
    CONFIG = {
        'train_graph_path': 'train_graph-homo.pt',
        'valid_graph_path': 'valid_graph-homo.pt',
        'output_dir': 'F:GATProtoMoE',
        'model_save_dir': 'F:\\GATProtoMoE\\models',
        'prototype_export_dir': 'F:\\GATProtoMoE\\prototype_samples',
        'num_prototypes': 10,
        'num_experts': None,
    ...
    try:
        ...
        best_fold_model = OptimizedGATProtoMoE(
        ...
        print("\n=== Final Full Training ===")
        final_model = OptimizedGATProtoMoE(
            input_dim=input_dim, out_channels=128,
            num_prototypes=CONFIG['num_prototypes'], device=device
        ).to(device)
