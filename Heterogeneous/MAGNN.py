...
from torch_geometric.nn import HANConv, Linear, GATConv
...
# ==========================================
# [Hetero Graph Augmentation]
# ==========================================
class HeteroGraphAugmentation:
    @staticmethod
    def edge_dropout(edge_index_dict, drop_rate=0.1):
        if drop_rate <= 0: return edge_index_dict
        new_dict = {}
        for k, v in edge_index_dict.items():
            mask = torch.rand(v.size(1), device=v.device) > drop_rate
            new_dict[k] = v[:, mask]
        return new_dict

    @staticmethod
    def feature_noise(x_dict, noise_scale=0.1):
        if noise_scale <= 0: return x_dict
        new_dict = {}
        for k, v in x_dict.items():
            new_dict[k] = v + torch.randn_like(v) * noise_scale
        return new_dict

    @staticmethod
    def feature_masking(x_dict, mask_rate=0.1):
        if mask_rate <= 0: return x_dict
        new_dict = {}
        for k, v in x_dict.items():
            mask = torch.rand(v.size(), device=v.device) > mask_rate
            new_dict[k] = v * mask.float()
        return new_dict

    @staticmethod
    def node_dropping(x_dict, edge_index_dict, drop_rate=0.1, target_node='patient'):
        if drop_rate <= 0: return x_dict, edge_index_dict
        new_x = {k: v.clone() for k, v in x_dict.items()}
        new_edge = {k: v.clone() for k, v in edge_index_dict.items()}

        num_nodes = x_dict[target_node].size(0)
        keep_mask = torch.rand(num_nodes, device=x_dict[target_node].device) > drop_rate
        new_x[target_node] = x_dict[target_node] * keep_mask.unsqueeze(1).float()

        for k, v in edge_index_dict.items():
            src_type, _, dst_type = k
            row, col = v
            edge_mask = torch.ones(v.size(1), dtype=torch.bool, device=v.device)
            if src_type == target_node:
                edge_mask = edge_mask & keep_mask[row]
            if dst_type == target_node:
                edge_mask = edge_mask & keep_mask[col]
            new_edge[k] = v[:, edge_mask]
...
def calculate_metrics(y_true, probs, return_threshold=False):
    ...
    if np.isnan(probs).any(): probs = np.nan_to_num(probs, nan=0.0)
    ...
    metrics = {
        ...
        'G-Mean': np.sqrt(
            precision_score(y_true, y_pred, zero_division=0) * recall_score(y_true, y_pred, zero_division=0)),
...
def plot_comparison_distribution(raw_features, learned_embeddings, labels, proto_ids, save_path, method='t-SNE'):
    ...
    # Subplot 3 & 4
    ax = axes[1, 0]
    ax.scatter(learned_2d[labels_subset == 0, 0], learned_2d[labels_subset == 0, 1], c='blue', alpha=0.3, s=5,
               label='Negative')
    ax.scatter(learned_2d[labels_subset == 1, 0], learned_2d[labels_subset == 1, 1], c='red', alpha=0.6, s=10,
               label='Positive')
    ...
    for i, pid in enumerate(unique_protos):
        ...
        if len(points) >= 3:
            try:
                ...
                poly = MplPolygon(hull_points, closed=True, facecolor=color, alpha=0.2, edgecolor=color, linewidth=2)
                ax.add_patch(poly)
            except Exception:
                pass
...
def augment_hetero_data(data, device='cpu'):

    def to_scipy_sparse(edge_index, num_src, num_dst):
        row = edge_index[0].cpu().numpy()
        col = edge_index[1].cpu().numpy()
        data_ones = np.ones(len(row))
        return sp.csr_matrix((data_ones, (row, col)), shape=(num_src, num_dst))

    def from_scipy_sparse_threshold(adj, threshold=1):
        adj = adj.copy()
        if threshold > 1:
            adj.data[adj.data < threshold] = 0
            adj.eliminate_zeros()
        adj_coo = adj.tocoo()
        row = torch.from_numpy(adj_coo.row).long()
        col = torch.from_numpy(adj_coo.col).long()
        if len(row) == 0: return torch.empty((2, 0), dtype=torch.long).to(device)
        return torch.stack([row, col], dim=0).to(device)

    num_patients = data['patient'].num_nodes if 'patient' in data.node_types else 0
    num_drugs = data['drug'].num_nodes if 'drug' in data.node_types else 0
    num_genes = data['gene'].num_nodes if 'gene' in data.node_types else 0
    num_phenotypes = data['phenotype'].num_nodes if 'phenotype' in data.node_types else 0

    if ('patient', 'takes', 'drug') in data.edge_types:
        adj_p_d = to_scipy_sparse(data['patient', 'takes', 'drug'].edge_index, num_patients, num_drugs)
    else:
        return data

    if ('drug', 'interacts_with', 'gene') in data.edge_types:
        adj_d_g = to_scipy_sparse(data['drug', 'interacts_with', 'gene'].edge_index, num_drugs, num_genes)
        adj_p_g = adj_p_d @ adj_d_g
        adj_g_p = adj_p_g.transpose()
        data['gene', 'associated_with_patient', 'patient'].edge_index = from_scipy_sparse_threshold(adj_g_p,
                                                                                                    threshold=1)

        if ('gene', 'associated_with', 'phenotype') in data.edge_types:
            adj_g_ph = to_scipy_sparse(data['gene', 'associated_with', 'phenotype'].edge_index, num_genes,
                                       num_phenotypes)
            adj_p_ph = adj_p_g @ adj_g_ph
            target_max_edges = 1_200_000
            current_threshold = 2
            temp_adj = adj_p_ph.copy()
            while temp_adj.nnz > target_max_edges and current_threshold < 50:
                current_threshold += 1
                mask = temp_adj.data >= current_threshold
                temp_adj.data[~mask] = 0
                temp_adj.eliminate_zeros()
            adj_ph_p = temp_adj.transpose()
            data['phenotype', 'risk_for', 'patient'].edge_index = from_scipy_sparse_threshold(adj_ph_p,
...
def get_in_channels_dict(hetero_data):
    ...
    for node_type in hetero_data.node_types:
        if hasattr(hetero_data[node_type], 'x') and hetero_data[node_type].x is not None:
            in_channels[node_type] = hetero_data[node_type].x.size(1)
        elif hasattr(hetero_data[node_type], 'num_nodes') and hetero_data[node_type].num_nodes is not None:
            if hetero_data[node_type].num_nodes > 0:
                in_channels[node_type] = hetero_data[node_type].num_nodes
    return in_channels


def prepare_hetero_data(hetero_data):
    for node_type in hetero_data.node_types:
        if hetero_data[node_type].x is None:
            num_nodes = hetero_data[node_type].num_nodes
            hetero_data[node_type].x = torch.eye(num_nodes)
    return hetero_data
...
class FeatureTokenizer(nn.Module):
    ...
    def forward(self, x):
        return x.unsqueeze(-1) * self.feature_weights.unsqueeze(0) + self.feature_bias.unsqueeze(0)
...
class FullTabTransformerExpert(nn.Module):
    def __init__(self, num_features, output_dim=2, dim=64, depth=2, heads=4, dropout=0.3):
        ...
        self.mlp_head = nn.Sequential(nn.LayerNorm(dim), nn.Linear(dim, dim // 2), nn.GELU(),
                                      nn.Dropout(dropout), nn.Linear(dim // 2, output_dim))

    def _forward_batch(self, x):
        tokens = self.tokenizer(x)
        cls_tokens = self.cls_token.expand(x.size(0), -1, -1)
        x_out = self.transformer(torch.cat((cls_tokens, tokens), dim=1))
        return self.mlp_head(x_out[:, 0, :])
    ...
    def forward(self, x):
        ...
        if x.size(0) > BATCH_SIZE:
            ...
            for i in range(0, x.size(0), BATCH_SIZE):
                ...
                if self.training:
                    if not chunk.requires_grad: chunk.requires_grad_()
                    out = cp.checkpoint(self._forward_batch, chunk, use_reentrant=False)
                else:
                    out = self._forward_batch(chunk)
                outs.append(out)
...
class FTTransformerBlock(nn.Module):
    def __init__(self, dim, heads=8, ff_mult=4, dropout=0.3):
        super().__init__()
...
class FTTransformerExpert(nn.Module):
    def __init__(self, num_features, output_dim=2, dim=64, depth=3, heads=8, dropout=0.3):
        super().__init__()
        self.expert_type = "FTTransformer"
        self.expert_confidence = 1.0
        self.feature_embeddings = nn.ModuleList([nn.Sequential(nn.Linear(1, dim), nn.ReLU(), nn.Linear(dim, dim))
                                                 for _ in range(num_features)])
        self.cls_token = nn.Parameter(torch.randn(1, 1, dim))
        self.layers = nn.ModuleList([FTTransformerBlock(dim, heads, dropout=dropout) for _ in range(depth)])
        self.head = nn.Sequential(nn.LayerNorm(dim), nn.Linear(dim, dim // 2), nn.GELU(),
                                  nn.Linear(dim // 2, output_dim))

    def _forward_batch(self, x):
        tokens = torch.stack([self.feature_embeddings[i](x[:, i:i + 1]) for i in range(len(self.feature_embeddings))],
                             dim=1)
        x_seq = torch.cat([self.cls_token.expand(x.size(0), -1, -1), tokens], dim=1)
        for layer in self.layers: x_seq = layer(x_seq)
        return self.head(x_seq[:, 0, :])
...
class DeepResidualExpert(nn.Module):
    def __init__(self, input_dim, output_dim=2, hidden_dims=[128, 64], expert_name="DeepRes"):
        super().__init__()
        self.expert_type = expert_name
        self.expert_confidence = 1.0
        self.net = nn.Sequential(nn.Linear(input_dim, hidden_dims[0]), nn.LayerNorm(hidden_dims[0]), nn.ReLU(),
                                 nn.Linear(hidden_dims[0], hidden_dims[1]), nn.LayerNorm(hidden_dims[1]), nn.ReLU(),
...
class SpecificMetaPathExpert(nn.Module):
    def __init__(self, in_channels_dict, metadata, out_channels=2, hidden_dim=64, heads=2, num_layers=2, dropout=0.3,
                 edge_type=None, expert_name="MetaExpert"):
        super().__init__()
        self.expert_type = expert_name
        self.expert_confidence = 1.0
        self.edge_type = edge_type
        self.input_projs = nn.ModuleDict({nt: nn.Linear(dim, hidden_dim) for nt, dim in in_channels_dict.items()})
        self.han_layers = nn.ModuleList(
            [HANConv(hidden_dim, hidden_dim, heads=heads, metadata=metadata, dropout=dropout) for _ in
             range(num_layers)])
        self.classifier = nn.Sequential(nn.Linear(hidden_dim, out_channels))

    def forward(self, x_dict, edge_index_dict):
        curr_x = {nt: F.elu(self.input_projs[nt](x)) for nt, x in x_dict.items()}
        edges = {
            self.edge_type: edge_index_dict[self.edge_type]} if self.edge_type in edge_index_dict else edge_index_dict

        for han in self.han_layers:
            out_dict = han(curr_x, edges)
            for nt in out_dict:
                if out_dict[nt] is not None:
                    if nt in curr_x:
                        curr_x[nt] = curr_x[nt] + F.elu(out_dict[nt])
                    else:
                        curr_x[nt] = F.elu(out_dict[nt])

        return self.classifier(curr_x['patient'])
...
# MAGNN Encoder
# ==========================================
class MAGNNConv(nn.Module):
    def __init__(self, in_channels, out_channels, heads, metadata, dropout=0.3):
        super().__init__()
        self.heads = heads
        self.gat_layers = nn.ModuleDict()
        for et in metadata[1]:
            self.gat_layers["__".join(et)] = GATConv(in_channels, out_channels // heads, heads=heads, dropout=dropout,
                                                     add_self_loops=False)
        self.semantic_att = nn.Linear(out_channels, 1, bias=False)
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        for et in edge_index_dict:
            src, rel, dst = et
            edge_key = "__".join(et)
            if edge_key in self.gat_layers:
                out = F.elu(self.gat_layers[edge_key]((x_dict[src], x_dict[dst]), edge_index_dict[et]))
                if dst not in semantic_embeddings: semantic_embeddings[dst] = []
                semantic_embeddings[dst].append(out)

        out_dict = {}
        for nt, embs in semantic_embeddings.items():
            stack_emb = torch.stack(embs, dim=0)
            att = F.softmax(torch.tanh(self.semantic_att(stack_emb)), dim=0)
            out_dict[nt] = torch.sum(stack_emb * att, dim=0)
        return out_dict


class DeepMAGNNEncoder(nn.Module):
    def __init__(self, in_channels_dict, hidden_channels, out_channels, metadata, heads=4, num_layers=3, dropout=0.3):
        super().__init__()
        self.input_projs = nn.ModuleDict({nt: nn.Linear(dim, hidden_channels) for nt, dim in in_channels_dict.items()})
        self.magnn_layers = nn.ModuleList(
            [MAGNNConv(hidden_channels, hidden_channels, heads, metadata, dropout) for _ in range(num_layers)])
        self.norms = nn.ModuleList([nn.LayerNorm(hidden_channels) for _ in range(num_layers)])
        self.patient_res_proj = nn.Linear(in_channels_dict['patient'], hidden_channels)
        self.final_proj = nn.Linear(hidden_channels, out_channels)

    def forward(self, x_dict, edge_index_dict, edge_dropout=0.0):
        x_curr = {nt: F.relu(self.input_projs[nt](x)) for nt, x in x_dict.items()}
        for i, conv in enumerate(self.magnn_layers):
            out_dict = conv(x_curr, edge_index_dict)
            for nt, out in out_dict.items():
                x_curr[nt] = self.norms[i](x_curr[nt] + out)

        patient_emb = x_curr['patient'] + self.patient_res_proj(x_dict['patient'])
        return self.final_proj(patient_emb)
...
class FullWeightRouter(nn.Module):
    def __init__(self, input_dim, num_experts, num_prototypes, hidden_dim=64, dropout=0.3):
        ...
        self.prototype_to_expert_map = nn.Parameter(torch.randn(num_prototypes, num_experts) * 0.5)
        self.router_net = nn.Sequential(nn.Linear(input_dim + num_prototypes, hidden_dim), nn.ReLU(),
    ...
    def forward(self, embeddings, proto_sims):
        ...
        if self.training:
            updated_ema = self.usage_decay * self.expert_usage_ema + (1 - self.usage_decay) * expert_weights.mean(
                0).detach()
...
class FullWeightedAggregator(nn.Module):
    def __init__(self, embedding_dim, expert_output_dim, num_experts, hidden_dim=128, dropout=0.2):
        ...
        self.mlp = nn.Sequential(nn.Linear(self.input_dim, hidden_dim), nn.LayerNorm(hidden_dim), nn.ReLU(),
                                 nn.Dropout(dropout), nn.Linear(hidden_dim, hidden_dim // 2), nn.ReLU(),
                                 nn.Dropout(dropout), nn.Linear(hidden_dim // 2, expert_output_dim))
    ...
    def forward(self, z, all_expert_outputs, expert_weights):
        ...
        flattened = weighted_outputs.view(z.size(0), -1)
        agg_input = torch.cat([z, flattened], dim=1)
        return self.mlp(agg_input)


# ==========================================
# Prototype Layer & Loss Functions
# ==========================================
class ImprovedPrototypeLayer(nn.Module):
    def __init__(self, embedding_dim, num_prototypes, diversity_lambda=0.5, scattering_lambda=0.5):
        ...
        self.inv_temperature = nn.Parameter(torch.tensor(1.0))
    ...
    def forward(self, embeddings):
        ...
        assignment_probs = F.softmax(similarities * self.inv_temperature, dim=1)
    ...
    def print_prototype_stats(self):
        ...
        for i in range(self.num_prototypes):
            ...
            print(f"{i}: {cnt} ({cnt / total * 100:.1f}%)")
...
def expert_diversity_loss(expert_outputs, temperature=0.5):
    ...
    if num_experts < 2: return torch.tensor(0.0, device=expert_outputs[0].device)
    diversity_loss = 0
    count = 0
...
# Main Model
# ==========================================
class OptimizedMAGNNProtoMoE(nn.Module):
    def __init__(self, in_channels_dict, metadata, out_channels=128, num_prototypes=10, num_experts=6, dropout=0.3,
                 device='cuda'):
        super().__init__()
        self.num_prototypes = num_prototypes
        self.device = device
        self.encoder = DeepMAGNNEncoder(in_channels_dict, 128, out_channels, metadata)
        self.prototype_layer = ImprovedPrototypeLayer(out_channels, num_prototypes)

        raw_dim = in_channels_dict['patient']

        self.experts = nn.ModuleList([
            LinearBaselineExpert(out_channels),
            DeepResidualExpert(out_channels),
            SpecificMetaPathExpert(in_channels_dict, metadata, edge_type=('drug', 'taken_by', 'patient'),
                                   expert_name="DrugExpert"),
            SpecificMetaPathExpert(in_channels_dict, metadata, edge_type=('phenotype', 'risk_for', 'patient'),
                                   expert_name="PhenotypeExpert"),
            FullTabTransformerExpert(raw_dim),
            FTTransformerExpert(raw_dim)
        ])

        self.num_experts = len(self.experts)

        self.router = FullWeightRouter(out_channels, self.num_experts, num_prototypes)
        self.aggregator = FullWeightedAggregator(out_channels, 2, self.num_experts)

        self.task_loss_fn = LabelSmoothingLoss()
        ...
        print(f"\nInitialized OptimizedMAGNNProtoMoE (Prototypes={num_prototypes}, Experts={self.num_experts})")

    def initialize_prototypes_with_data(self, data):
        self.eval()
        with torch.no_grad():
            embs = self.encoder(data.x_dict, data.edge_index_dict)
            self.prototype_layer.initialize_with_kmeans(embs, mask=data['patient'].train_mask, labels=data['patient'].y)
        self.train()

    def forward(self, data, edge_dropout=0.0):
        patient_embs = self.encoder(data.x_dict, data.edge_index_dict, edge_dropout=edge_dropout)
        raw_x = data['patient'].x

        _, proto_sims, assign_probs = self.prototype_layer(patient_embs)
        expert_weights, exp_logits = self.router(patient_embs, proto_sims)

        all_exp_outs = []
        for exp in self.experts:
            if "Transformer" in exp.expert_type:
                all_exp_outs.append(exp(raw_x))
            elif "Expert" in exp.expert_type:
                all_exp_outs.append(exp(data.x_dict, data.edge_index_dict))
            else:
                all_exp_outs.append(exp(patient_embs))

        final_out = self.aggregator(patient_embs, all_exp_outs, expert_weights)
        return {
            'final_logits': final_out, 'embeddings': patient_embs, 'prototypes': self.prototype_layer.prototypes,
            'expert_logits': exp_logits, 'expert_weights': expert_weights, 'all_expert_outputs': all_exp_outs,
            'prototype_similarities': proto_sims, 'assignment_probs': assign_probs
        }

    def compute_loss(self, out, labels, mask, stage=3):
        task_loss = self.task_loss_fn(out['final_logits'][mask], labels[mask])
    ...
    def print_detailed_analysis(self, data, export_dir=None):
        ...
        if export_dir:
            ...
            self.eval()
            with torch.no_grad():
                raw_x = data['patient'].x.detach().cpu().numpy()
                out = self(data)
                learned_x = out['embeddings'].detach().cpu().numpy()
                labels = data['patient'].y.detach().cpu().numpy()
...
# ==========================================
def evaluate_magnn_proto_moe(model, data):
    model.eval()
    with torch.no_grad():
        out = model(data)
        if hasattr(data['patient'], 'val_mask') and data['patient'].val_mask is not None:
            mask = data['patient'].val_mask
        else:
            mask = torch.ones(data['patient'].num_nodes, dtype=torch.bool, device=data['patient'].x.device)
        probs = F.softmax(out['final_logits'][mask], dim=1)[:, 1].cpu().numpy()
        y = data['patient'].y[mask].cpu().numpy()
...
def evaluate_individual_experts(model, data):
    ...
    with torch.no_grad():
        ...
        if hasattr(data['patient'], 'val_mask') and data['patient'].val_mask is not None:
            ...
            mask = torch.ones(data['patient'].num_nodes, dtype=torch.bool, device=raw_x.device)
        y = data['patient'].y[mask].cpu().numpy()
        perfs = {}
        ...
        for i, exp in enumerate(model.experts):
            if "Transformer" in exp.expert_type:
                ...
                logits = exp(data.x_dict, data.edge_index_dict)[mask]
            else:
...
def evaluate_on_validation_set(model, valid_data, save_path=None):
    model.eval()
    with torch.no_grad():
        try:
            out = model(valid_data)
            mask = torch.ones(valid_data['patient'].num_nodes, dtype=torch.bool, device=valid_data['patient'].x.device)

            probs = F.softmax(out['final_logits'][mask], dim=1)[:, 1].cpu().numpy()
            y = valid_data['patient'].y[mask].cpu().numpy()

            if save_path:
                os.makedirs(os.path.dirname(save_path), exist_ok=True)
                np.savez(save_path, y_true=y, y_prob=probs)
                print(f"DCA数据已保存至: {save_path}")
...
def enhanced_three_stage_training(model, train_data, epochs_per_stage=500, lr=0.001):
    ...
    y_true = train_data['patient'].y
    train_mask = train_data['patient'].train_mask
    ...
    for epoch in range(1, epochs_per_stage + 1):
        ...
        aug_edge_index = HeteroGraphAugmentation.edge_dropout(train_data.edge_index_dict, drop_rate=0.2)
        aug_x = HeteroGraphAugmentation.feature_noise(train_data.x_dict, noise_scale=0.1)
        aug_data = train_data.clone()
        aug_data.edge_index_dict = aug_edge_index
        for k in aug_x: aug_data[k].x = aug_x[k]
        ...
        if epoch % 10 == 0:
            val_metrics = evaluate_magnn_proto_moe(model, train_data)
    ...
    for epoch in range(1, epochs_per_stage + 1):
        ...
        aug_x1 = HeteroGraphAugmentation.feature_masking(train_data.x_dict, mask_rate=0.15)
        aug_edge1 = HeteroGraphAugmentation.edge_dropout(train_data.edge_index_dict, drop_rate=0.2)
        aug_data1 = train_data.clone()
        aug_data1.edge_index_dict = aug_edge1
        for k in aug_x1: aug_data1[k].x = aug_x1[k]

        aug_x2, aug_edge2 = HeteroGraphAugmentation.node_dropping(train_data.x_dict, train_data.edge_index_dict,
                                                                  drop_rate=0.1, target_node='patient')
        aug_x2 = HeteroGraphAugmentation.feature_noise(aug_x2, noise_scale=0.1)
        aug_data2 = train_data.clone()
        aug_data2.edge_index_dict = aug_edge2
        for k in aug_x2: aug_data2[k].x = aug_x2[k]
        ...
        del out1, out2, p, q, loss1, loss2, total_loss, kl_loss
        torch.cuda.empty_cache()
        ...
        if epoch % 10 == 0:
            ...
            if early_stopping.early_stop:
                ...
                break
            print(f"  Stage 3 | Ep {epoch}: Loss Valid F1 {val_metrics['F1']:.4f}")
    ...
    return evaluate_magnn_proto_moe(model, train_data)


def train_magnn_proto_moe_cv(train_data, save_dir='model', n_folds=5, epochs_per_stage=500, dynamic_k=None):
    num_nodes = train_data['patient'].num_nodes
    y_np = train_data['patient'].y.cpu().numpy()
    skf = StratifiedKFold(n_splits=n_folds, shuffle=True, random_state=42)
    device = train_data['patient'].x.device
    results = []
    all_metrics = {'F1': [], 'MCC': [], 'AUC-ROC': [], 'Precision': [], 'Recall': [], 'AUPRC': [], 'G-Mean': []}
    model_paths = []

    final_k = dynamic_k if dynamic_k is not None else 10
    in_channels_dict = get_in_channels_dict(train_data)
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(num_nodes), y_np)):
        ...
        data_fold['patient'].train_mask = train_mask
        data_fold['patient'].val_mask = val_mask

        model = OptimizedMAGNNProtoMoE(
            in_channels_dict=in_channels_dict, metadata=metadata,
            out_channels=128, num_prototypes=final_k, num_experts=6, device=device
        ).to(device)
    ...
    return {'fold_results': results, 'summary': {k: (np.mean(v), np.std(v)) for k, v in all_metrics.items()},
            'model_paths': model_paths}


def find_optimal_prototypes_elbow(data, in_channels_dict, metadata, device, output_dir, k_range=(2, 21)):
    print("\n[Elbow Method]")
    temp_encoder = DeepMAGNNEncoder(
        in_channels_dict=in_channels_dict, hidden_channels=128, out_channels=128,
        metadata=metadata, num_layers=3, dropout=0.0
    ).to(device)
    temp_encoder.eval()

    with torch.no_grad():
        embeddings = temp_encoder(data.x_dict, data.edge_index_dict).cpu().numpy()
...
def main_magnn_proto_moe():
    CONFIG = {
        'train_graph_path': 'train_graph-Hetero.pt',
        'valid_graph_path': 'valid_graph-Hetero.pt',
        'output_dir': 'F:\\MAGNNProtoMoE',
        'model_save_dir': 'F:\\MAGNNProtoMoE\\models',
        'prototype_export_dir': 'F:\\MAGNNProtoMoE\\prototype_samples',
        'num_prototypes': 10,
        'num_experts': 6,
    ...
    try:
        print("Loading Hetero Graph Data...")
        train_data = torch.load(CONFIG['train_graph_path'], weights_only=False).to(device)
        train_data = prepare_hetero_data(train_data)
        train_data = augment_hetero_data(train_data, device)
        ...
        if CONFIG['use_elbow_method']:
            optimal_k = find_optimal_prototypes_elbow(
                train_data, in_channels_dict, metadata, device, CONFIG['output_dir']
            )
            if optimal_k < CONFIG['num_experts']:
                optimal_k = CONFIG['num_experts']

            CONFIG['num_prototypes'] = optimal_k

        cv_res = train_magnn_proto_moe_cv(
        ...
        best_fold_model = OptimizedMAGNNProtoMoE(
            in_channels_dict=in_channels_dict, metadata=metadata, out_channels=128,
            num_prototypes=CONFIG['num_prototypes'], num_experts=CONFIG['num_experts'], device=device
        ).to(device)
        ...
        valid_data = prepare_hetero_data(valid_data)
        valid_data = augment_hetero_data(valid_data, device)
        valid_data['patient'].val_mask = torch.ones(valid_data['patient'].num_nodes, dtype=torch.bool, device=device)

        print("\n=== Final Full Training ===")
        final_model = OptimizedMAGNNProtoMoE(
            in_channels_dict=in_channels_dict, metadata=metadata, out_channels=128,
            num_prototypes=CONFIG['num_prototypes'], num_experts=CONFIG['num_experts'], device=device
        ).to(device)

        train_data['patient'].train_mask = torch.ones(train_data['patient'].num_nodes, dtype=torch.bool, device=device)
        train_data['patient'].val_mask = train_data['patient'].train_mask

        enhanced_three_stage_training(final_model, train_data, epochs_per_stage=500)
        final_path = os.path.join(CONFIG['model_save_dir'], 'final_model.pth')
        torch.save(final_model.state_dict(), final_path)

        save_name = 'MAGNN_predictions.npz'
        metrics, thresh, exp_perf = evaluate_on_validation_set(
            final_model, valid_data,
