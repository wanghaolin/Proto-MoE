...
class HeteroGraphAugmentation:
    ...
    @staticmethod
    def node_dropping(x_dict, edge_index_dict, drop_rate=0.1):
...
def plot_comparison_distribution(raw_features, learned_embeddings, labels, proto_ids, save_path, method='t-SNE'):
    print(f"正在绘制 {method} 对比分布图 (Samples: {raw_features.shape[0]})...")
...
def augment_hetero_data(data, device='cpu'):
    ...
    if ('drug', 'interacts_with', 'gene') in data.edge_types:
        ...
        data['gene', 'associated_with_patient', 'patient'].edge_index = from_scipy_sparse_threshold(adj_g_p,
                                                                                                    threshold=1)
        print(f"  > Gene->Patient: {data['gene', 'associated_with_patient', 'patient'].edge_index.size(1)} ")
        ...
        if ('gene', 'associated_with', 'phenotype') in data.edge_types:
            ...
            current_nnz = temp_adj.nnz
            while current_nnz > target_max_edges and current_threshold < 50:
                current_threshold += 1
                mask = temp_adj.data >= current_threshold
                current_nnz = np.sum(mask)
            print(f"  > Phenotype->Patient (Threshold={current_threshold}): {current_nnz} 条")
            adj_ph_p = adj_p_ph.transpose()
...
class SpecificMetaPathExpert(nn.Module):
    def __init__(self, in_channels_dict, metadata, out_channels=2, hidden_dim=64, heads=2, num_layers=2, dropout=0.3,
        ...
        self.han_layers = nn.ModuleList(
            [HANConv(in_channels=hidden_dim, out_channels=hidden_dim, heads=heads, metadata=metadata, dropout=dropout)
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        for i, han in enumerate(self.han_layers):
            out_dict = han(curr_x, specific_edge_dict)
            for nt in out_dict:
                out = out_dict[nt]
...
# HAN Graph Encoder
# ==========================================
class TrueDeepHANEncoder(nn.Module):
    def __init__(self, in_channels_dict, hidden_channels, out_channels, metadata, heads=4, num_layers=3, dropout=0.3):
        ...
        for i in range(num_layers):
            self.convs.append(
                HANConv(in_channels=hidden_channels, out_channels=hidden_channels, heads=heads, metadata=metadata,
                        dropout=dropout))
    ...
    def forward(self, x_dict, edge_index_dict, edge_dropout=0.0):
        if self.training and edge_dropout > 0.0:
            ...
            for k, v in edge_index_dict.items():
                edge_index_dict_curr[k], _ = dropout_edge(v, p=edge_dropout, training=True)
        else:
            edge_index_dict_curr = edge_index_dict

        raw_patient = x_dict['patient']
        x_dict_curr = {nt: F.dropout(F.relu(self.input_projs[nt](x)), p=self.dropout, training=self.training) for nt, x
                       in x_dict.items()}

        for i, conv in enumerate(self.convs):
            out_dict = conv(x_dict_curr, edge_index_dict_curr)
            for nt, out in out_dict.items():
                ...
                if nt in x_dict_curr:
                    x_dict_curr[nt] = x_dict_curr[nt] + out
                else:
                    x_dict_curr[nt] = out

        patient_emb = x_dict_curr['patient'] + self.patient_res_proj(raw_patient)
...
class ImprovedPrototypeLayer(nn.Module):
    def __init__(self, embedding_dim, num_prototypes, diversity_lambda=0.5, scattering_lambda=0.5):
        ...
        self.inv_temperature = nn.Parameter(torch.tensor(1.0))  # [Updated] like MAGNN
    ...
    def print_prototype_stats(self):
        ...
        for i in range(self.num_prototypes):
            ...
            print(f"Prototype {i}: {cnt} ({cnt / total * 100:.1f}%)")
...
# Main Model
# ==========================================
class OptimizedHANProtoMoE(nn.Module):
    def __init__(self, in_channels_dict, metadata, out_channels=128, num_prototypes=10, num_experts=6, dropout=0.3,
                 expert_output_dim=2, device='cuda'):
        super().__init__()
        self.num_prototypes = num_prototypes
        self.device = device

        # HAN Encoder
        self.encoder = TrueDeepHANEncoder(
            in_channels_dict=in_channels_dict, hidden_channels=128, out_channels=out_channels,
            metadata=metadata, heads=4, num_layers=3, dropout=dropout
        )
        ...
        self.experts = nn.ModuleList([
            LinearBaselineExpert(out_channels, expert_output_dim),
            DeepResidualExpert(out_channels, expert_output_dim),
            SpecificMetaPathExpert(in_channels_dict, metadata, expert_output_dim, hidden_dim=64, heads=4, num_layers=2,
                                   edge_type=('drug', 'taken_by', 'patient'), expert_name="DrugExpert"),
            SpecificMetaPathExpert(in_channels_dict, metadata, expert_output_dim, hidden_dim=64, heads=2, num_layers=2,
                                   edge_type=('phenotype', 'risk_for', 'patient'), expert_name="PhenotypeExpert"),
            FullTabTransformerExpert(num_features=raw_dim, output_dim=expert_output_dim),
            FTTransformerExpert(num_features=raw_dim, output_dim=expert_output_dim)
        ])

        self.num_experts = len(self.experts)
        self.router = FullWeightRouter(out_channels, self.num_experts, num_prototypes)
        self.aggregator = FullWeightedAggregator(out_channels, expert_output_dim, self.num_experts)
        ...
        print(f"\nInitialized OptimizedHANProtoMoE (Prototypes={num_prototypes}, Experts={self.num_experts})")
    ...
    def forward(self, data, edge_dropout=0.0):
        ...
        for exp in self.experts:
            ...
            elif "Expert" in exp.expert_type and ("Drug" in exp.expert_type or "Phenotype" in exp.expert_type):
...
# ==========================================
def evaluate_han_proto_moe(model, data):
    model.eval()
    with torch.no_grad():
        out = model(data)
...
def enhanced_three_stage_training(model, train_data, epochs_per_stage=500, lr=0.001):
    ...
    for epoch in range(1, epochs_per_stage + 1):
        ...
        if epoch % 10 == 0:
            val_metrics = evaluate_han_proto_moe(model, train_data)
    ...
    return evaluate_han_proto_moe(model, train_data)


def train_han_proto_moe_cv(train_data, save_dir='model', n_folds=5, epochs_per_stage=500, dynamic_k=None):
...
def find_optimal_prototypes_elbow(data, in_channels_dict, metadata, device, output_dir, k_range=(2, 21)):
    ...
    temp_encoder = TrueDeepHANEncoder(
        in_channels_dict=in_channels_dict, hidden_channels=128, out_channels=128,
        metadata=metadata, heads=4, num_layers=3, dropout=0.0
    ).to(device)
    temp_encoder.eval()
...
def main_han_proto_moe():
    CONFIG = {
        'train_graph_path': 'train_graph-Hetero.pt',
        'valid_graph_path': 'valid_graph-Hetero.pt',
        'output_dir': 'F:\\HANProtoMoE',
        'model_save_dir': 'F:\\HANProtoMoE\\models',
        'prototype_export_dir': 'F:\\HANProtoMoE\\prototype_samples',
        'num_prototypes': 10,
        'num_experts': 6,
    ...
    try:
        ...
        best_fold_model = OptimizedHANProtoMoE(
        ...
        print("\n=== Final Full Training ===")
        final_model = OptimizedHANProtoMoE(
            in_channels_dict=in_channels_dict, metadata=metadata, out_channels=128,
            num_prototypes=CONFIG['num_prototypes'], num_experts=CONFIG['num_experts'], device=device
        ).to(device)
