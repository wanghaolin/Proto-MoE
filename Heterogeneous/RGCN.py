...
from torch_geometric.nn import HANConv, Linear, HeteroConv, SAGEConv
...
def format_metrics_str(metrics):
    order = ['MCC', 'Precision', 'Recall', 'F1', 'AUC-ROC', 'AUPRC', 'G-Mean']
    parts = []
    for k in order:
        if k in metrics:
...
def plot_comparison_distribution(raw_features, learned_embeddings, labels, proto_ids, save_path, method='t-SNE'):
    ...
    # Subplot 4: Learned + Proto IDs + Convex Hull
    ax = axes[1, 1]
    for i, pid in enumerate(unique_protos):
        mask = proto_subset == pid
...
def augment_hetero_data(data, device='cpu'):
    ...
    if ('drug', 'interacts_with', 'gene') in data.edge_types:
        ...
        data['gene', 'associated_with_patient', 'patient'].edge_index = from_scipy_sparse_threshold(adj_g_p,
                                                                                                    threshold=1)
        print(f"  > Gene->Patient: {data['gene', 'associated_with_patient', 'patient'].edge_index.size(1)} edges")
...
class SpecificMetaPathExpert(nn.Module):
    def __init__(self, in_channels_dict, metadata, out_channels=2, hidden_dim=64, heads=2, num_layers=2, dropout=0.3,
        ...
        for _ in range(num_layers):
            self.han_layers.append(HANConv(in_channels=hidden_dim, out_channels=hidden_dim, heads=heads,
                                           metadata=metadata, dropout=dropout))
            self.linears.append(nn.Linear(hidden_dim, hidden_dim))
            self.norms.append(nn.LayerNorm(hidden_dim))
        self.classifier = nn.Sequential(nn.Linear(hidden_dim, hidden_dim), nn.ReLU(), nn.Dropout(dropout),
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        for node_type, x in x_dict.items():
            if node_type in self.input_projs:
                x_dict_proj[node_type] = F.elu(self.input_projs[node_type](x))
        specific_edge_dict = {}
        if self.edge_type in edge_index_dict:
            specific_edge_dict[self.edge_type] = edge_index_dict[self.edge_type]
        else:
            specific_edge_dict = edge_index_dict
        curr_x_dict = x_dict_proj
        for i, han in enumerate(self.han_layers):
            out_dict = han(curr_x_dict, specific_edge_dict)
            for nt in out_dict:
                out = out_dict[nt]
                ...
                if nt in curr_x_dict:
                    curr_x_dict[nt] = curr_x_dict[nt] + out
                else:
                    curr_x_dict[nt] = out
        return self.classifier(curr_x_dict['patient'])
...
# Encoder
# ==========================================
class TrueDeepRGCNEncoder(nn.Module):
    def __init__(self, in_channels_dict, hidden_channels, out_channels, metadata, num_layers=3, dropout=0.3):
        super().__init__()
        self.dropout = dropout
        ...
        self.input_projs = nn.ModuleDict()
        for node_type, in_dim in in_channels_dict.items():
            self.input_projs[node_type] = nn.Linear(in_dim, hidden_channels)
        self.convs = nn.ModuleList()
        self.norms = nn.ModuleList()
        self.linears = nn.ModuleList()
        edge_types = metadata[1]
        for i in range(num_layers):
            conv_dict = {edge_type: SAGEConv(hidden_channels, hidden_channels) for edge_type in edge_types}
    ...
    def forward(self, x_dict, edge_index_dict, edge_dropout=0.0):
        ...
        for node_type, x in x_dict.items():
            if node_type in self.input_projs:
                x_dict_curr[node_type] = F.dropout(F.relu(self.input_projs[node_type](x)), p=self.dropout,
        ...
        for i, conv in enumerate(self.convs):
            ...
            for node_type, out in out_dict.items():
                ...
                out = F.elu(out)
                out = F.dropout(out, p=self.dropout, training=self.training)
                if node_type in x_dict_curr:
                    x_dict_curr[node_type] = x_dict_curr[node_type] + out
                else:
                    x_dict_curr[node_type] = out
        gnn_out = x_dict_curr['patient']
        raw_out = self.patient_res_proj(raw_patient)
...
# Main Model
# ==========================================
class OptimizedRGCNProtoMoE(nn.Module):
    def __init__(self, in_channels_dict, metadata, out_channels=128, num_prototypes=10, num_experts=6,
        ...
        self.encoder = TrueDeepRGCNEncoder(in_channels_dict=in_channels_dict, hidden_channels=HIDDEN_DIM,
                                           out_channels=embedding_dim, metadata=metadata, num_layers=3, dropout=dropout)
        ...
        self.router = FullWeightRouter(embedding_dim, self.num_experts, num_prototypes)
        self.aggregator = FullWeightedAggregator(embedding_dim, expert_output_dim, self.num_experts)
        self.task_loss_fn = LabelSmoothingLoss(classes=2, smoothing=0.1)
        ...
        print(f"\nInitialized OptimizedRGCNProtoMoE (Prototypes={num_prototypes}, Experts={self.num_experts})")
    ...
    def forward(self, data, edge_dropout=0.0):
        ...
        for exp in self.experts:
            if isinstance(exp, (FullTabTransformerExpert, FTTransformerExpert)):
                all_expert_outputs.append(exp(raw_x))
            elif hasattr(exp, 'expert_type') and any(x in exp.expert_type for x in ["Drug", "Phenotype"]):
                all_expert_outputs.append(exp(data.x_dict, data.edge_index_dict))
            else:
                all_expert_outputs.append(exp(patient_embs))

        final_out = self.aggregator(patient_embs, all_expert_outputs, expert_weights)
        return {
            'final_logits': final_out, 'embeddings': patient_embs, 'prototypes': self.prototype_layer.prototypes,
            'expert_logits': exp_logits, 'expert_weights': expert_weights, 'all_expert_outputs': all_expert_outputs,
...
# ==========================================
def evaluate_rgcn_proto_moe(model, data):
    model.eval()
    with torch.no_grad():
        out = model(data)
...
def enhanced_three_stage_training(model, train_data, epochs_per_stage=500, lr=0.001):
    ...
    for epoch in range(1, epochs_per_stage + 1):
        ...
        if epoch % 10 == 0:
            val_metrics = evaluate_rgcn_proto_moe(model, train_data)
    ...
    return evaluate_rgcn_proto_moe(model, train_data)


def train_rgcn_proto_moe_cv(train_data, save_dir='model', n_folds=5, epochs_per_stage=500, dynamic_k=None):
...
def find_optimal_prototypes_elbow(data, in_channels_dict, metadata, device, output_dir, k_range=(2, 21)):
    print("\n[Elbow Method]")
    temp_encoder = TrueDeepRGCNEncoder(
...
def main_rgcn_proto_moe():
    CONFIG = {
        'train_graph_path': 'train_graph-Hetero.pt',
        'valid_graph_path': 'valid_graph-Hetero.pt',
        'output_dir': 'F:\\RGCNProtoMoE',
        'model_save_dir': 'F:\\RGCNProtoMoE\\models',
        'prototype_export_dir': 'F:\\RGCNProtoMoE\\prototype_samples',
        'num_prototypes': 10,
        'num_experts': 6,
    ...
    try:
        ...
        in_channels_dict = get_in_channels_dict(train_data)
        metadata = train_data.metadata()
        temp_experts_count = CONFIG['num_experts']
        ...
        cv_res = train_rgcn_proto_moe_cv(
        ...
        best_fold_model = OptimizedRGCNProtoMoE(
        ...
        print("\n=== Final Full Training ===")
        final_model = OptimizedRGCNProtoMoE(
            in_channels_dict=in_channels_dict, metadata=metadata, out_channels=128,
            num_prototypes=CONFIG['num_prototypes'], num_experts=CONFIG['num_experts'], device=device
        ).to(device)
        ...
        save_name = 'RGCN_predictions.npz'
