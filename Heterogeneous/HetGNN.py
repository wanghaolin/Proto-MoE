...
class FullTabTransformerExpert(nn.Module):
    def __init__(self, num_features, output_dim=2, dim=64, depth=2, heads=4, dropout=0.3):
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=dim, nhead=heads, dim_feedforward=dim * 2, dropout=dropout,
                                                   activation='gelu', batch_first=True)
        self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=depth)
        self.mlp_head = nn.Sequential(nn.LayerNorm(dim), nn.Linear(dim, dim // 2), nn.GELU(), nn.Dropout(dropout),
    ...
    def forward(self, x):
        ...
        if x.size(0) > BATCH_SIZE:
            ...
            for i in range(0, x.size(0), BATCH_SIZE):
                outs.append(self._forward_batch(x[i:i + BATCH_SIZE]))
...
class SpecificRelationExpert(nn.Module):
    def __init__(self, in_channels_dict, metadata, out_channels=2, hidden_dim=64, heads=None, num_layers=2, dropout=0.3,
                 edge_type=None, expert_name="RelationExpert"):
        super().__init__()
        self.expert_type = expert_name
        self.expert_confidence = 1.0
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        if self.edge_type not in edge_index_dict:
            return self.classifier(torch.zeros(x_dict['patient'].size(0), 64, device=x_dict['patient'].device))

        x_src = x_dict_proj[src_type]
        x_dst = x_dict_proj[dst_type]
        edge_index = edge_index_dict[self.edge_type]

        out = self.conv((x_src, x_dst), edge_index)
        out = self.lin(out)
        out = self.norm(out)
        out = F.elu(out)
...
class ImprovedPrototypeLayer(nn.Module):
    ...
    def print_prototype_stats(self):
        ...
        for i in range(self.num_prototypes):
            ...
            print(f"原型 {i}: {cnt} ({cnt / total * 100:.1f}%)")


class HetGNNEncoder(nn.Module):
    def __init__(self, in_channels_dict, hidden_channels, out_channels, metadata, heads=None, num_layers=3,
                 dropout=0.3):
        super().__init__()
        ...
        for i in range(num_layers):
            conv_dict = {edge_type: SAGEConv((-1, -1), hidden_channels) for edge_type in metadata[1]}
        ...
        print(f"Initialized HetGNNEncoder ({num_layers} layers, SAGEConv)")
...
class FullWeightRouter(nn.Module):
    def __init__(self, input_dim, num_experts, num_prototypes, hidden_dim=64, dropout=0.3):
        ...
        self.router_net = nn.Sequential(
            nn.Linear(input_dim + num_prototypes, hidden_dim), nn.ReLU(),
    ...
    def get_usage_imbalance_loss(self):
        ...
        # FIX: NaN protection, use 1e-5 instead of 1e-8
        return F.kl_div(torch.log(self.expert_usage_ema + 1e-5), target, reduction='batchmean')
...
class SupervisedContrastiveLoss(nn.Module):
    ...
    def forward(self, embeddings, labels):
        ...
        embeddings = F.normalize(embeddings, p=2, dim=1, eps=1e-5)
        ...
        anchor_dot_contrast = torch.clamp(anchor_dot_contrast, min=-50.0, max=50.0)
...
class UncertaintyWeightedLoss(nn.Module):
    ...
    def forward(self, losses_dict):
        ...
        for i, (name, loss) in enumerate(losses_dict.items()):
            safe_log_var = torch.clamp(self.log_vars[i], min=-10.0, max=10.0)
            precision = torch.exp(-safe_log_var)
            weighted_loss = precision * loss + safe_log_var
...
class EnhancedPrototypeScatteringLoss(nn.Module):
    ...
    def forward(self, prototypes):
        ...
        prototypes_norm = F.normalize(prototypes, p=2, dim=1, eps=1e-5)
...
# [Main Model]
# ==========================================
class OptimizedHetGNNProtoMoE(nn.Module):
    def __init__(self, in_channels_dict, metadata, out_channels=128, num_prototypes=10, num_experts=6,
        ...
        self.encoder = HetGNNEncoder(
            in_channels_dict=in_channels_dict, hidden_channels=GNN_HIDDEN, out_channels=embedding_dim,
            metadata=metadata, num_layers=3, dropout=dropout
        )
        ...
        self.experts = nn.ModuleList([
            ...
            SpecificRelationExpert(in_channels_dict, metadata, expert_output_dim, hidden_dim=64, num_layers=2,
                                   edge_type=('drug', 'taken_by', 'patient'), expert_name="DrugRelExpert"),
            SpecificRelationExpert(in_channels_dict, metadata, expert_output_dim, hidden_dim=64, num_layers=2,
                                   edge_type=('phenotype', 'risk_for', 'patient'), expert_name="PhenotypeRelExpert"),
        ...
        print(f"\nOptimizedHetGNNProtoMoE (Prototypes={num_prototypes}, Experts={self.num_experts})")
    ...
    def compute_loss(self, out, labels, mask, stage=3):
        ...
        proto_entropy = -torch.sum(avg_proto_usage * torch.log(avg_proto_usage + 1e-5))
    ...
    def forward(self, data, edge_dropout=0.0):
        ...
        expert_weights, expert_logits = self.router(patient_embs, proto_sims)

        all_outs = []
        for exp in self.experts:
            if isinstance(exp, (FullTabTransformerExpert, FTTransformerExpert)):
                all_outs.append(exp(raw_x))
            elif "RelExpert" in exp.expert_type:
                all_outs.append(exp(data.x_dict, data.edge_index_dict))
            else:
                all_outs.append(exp(patient_embs))
        final_out = self.aggregator(patient_embs, all_outs, expert_weights)

        return {
            'final_logits': final_out,
            'embeddings': patient_embs,
...
def evaluate_individual_experts(model, data):
    model.eval()
    with torch.no_grad():
        patient_embs = model.encoder(data.x_dict, data.edge_index_dict)
        raw_x = data['patient'].x

        if hasattr(data['patient'], 'val_mask') and data['patient'].val_mask is not None:
            mask = data['patient'].val_mask
        else:
            mask = torch.ones(data['patient'].num_nodes, dtype=torch.bool, device=patient_embs.device)
        y = data['patient'].y[mask].cpu().numpy()
        perfs = {}
...
def enhanced_three_stage_training(model, train_data, epochs_per_stage=500, lr=0.001):
    ...
    for epoch in range(1, epochs_per_stage + 1):
        ...
        with torch.amp.autocast(device_type='cuda', dtype=torch.bfloat16):
            out = model(train_data, edge_dropout=0.2)
    ...
    for epoch in range(1, epochs_per_stage + 1):
        ...
        with torch.amp.autocast(device_type='cuda', dtype=torch.bfloat16):
            out1 = model(train_data, edge_dropout=0.2)
            loss1, _ = model.compute_loss(out1, y_true, train_mask, stage=3)

            out2 = model(train_data, edge_dropout=0.2)
            ...
            kl_loss = 0.5 * (F.kl_div(torch.log(p + 1e-5), q, reduction='batchmean') + F.kl_div(torch.log(q + 1e-5), p,
                                                                                                reduction='batchmean'))
...
def train_han_proto_moe_cv(train_data, save_dir='model', n_folds=5, epochs_per_stage=500, dynamic_k=None):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(num_patients), y_np)):
        print(f"\n=== Fold {fold + 1} ===")
        data_fold = train_data.clone()
        train_mask = torch.zeros(num_patients, dtype=torch.bool, device=device)
        train_mask[train_idx] = True
        val_mask = torch.zeros(num_patients, dtype=torch.bool, device=device)
        val_mask[val_idx] = True
        data_fold['patient'].train_mask = train_mask
        data_fold['patient'].val_mask = val_mask

        model = OptimizedHetGNNProtoMoE(
            in_channels_dict=in_channels_dict, metadata=metadata, out_channels=128,
            num_prototypes=final_k, num_experts=6, device=device
        ).to(device)

        enhanced_three_stage_training(model, data_fold, epochs_per_stage=epochs_per_stage)
        metrics = evaluate_han_proto_moe(model, data_fold)
...
def main_han_proto_moe():
    CONFIG = {
        ...
        'output_dir': 'F:\\HetGNNProtoMoE',
        'model_save_dir': 'F:\\HetGNNProtoMoE\\models',
        'prototype_export_dir': 'F:\\HetGNNProtoMoE\\prototype_samples',
        'num_prototypes': 10,
        'num_experts': 6,
    ...
    try:
        ...
        best_fold_model = OptimizedHetGNNProtoMoE(
            in_channels_dict=in_channels_dict, metadata=metadata, out_channels=128,
        ...
        print("\n=== Final Full Training ===")
        final_model = OptimizedHetGNNProtoMoE(
            in_channels_dict=in_channels_dict, metadata=metadata, out_channels=128,
            num_prototypes=CONFIG['num_prototypes'], num_experts=CONFIG['num_experts'], device=device
        ).to(device)
        ...
        save_name = 'HetGNN_predictions.npz'
